...
public class App 
{
    public static void main( String[] args )
    {
      ...
      final  int n = movies.size();
        ...
        /*
        JavaPairRDD<Integer, Iterable<Tuple2<Integer, Double>>> utility1=ratings.groupByKey();
        JavaPairRDD<Integer,List<Double>> utility2=utility1.mapPartitionsToPair(new PairFlatMapFunction<Iterator<Tuple2<Integer,Iterable<Tuple2<Integer, Double>>>>,Integer,List<Double>>()
        		{
					...
					public Iterable<Tuple2<Integer, List<Double>>> call(
							Iterator<Tuple2<Integer, Iterable<Tuple2<Integer, Double>>>> t)
							throws Exception {
						...
						while(t.hasNext())
	  					{
							ArrayList<Double> userratings = new ArrayList<Double>();
	  						...
	  						Tuple2<Integer, Iterable<Tuple2<Integer, Double>>> record = t.next();
	  						int userid=record._1;
	  						while(record._2.iterator().hasNext())
	  						{
	  							Tuple2<Integer, Double> moviendreview = record._2.iterator().next();
	  							int movieid=moviendreview._1;
	  							double rating=moviendreview._2;
	  							int index=movies.indexOf(movieid);
        ...
        utility2.saveAsTextFile("utility2");
        */
     ...
     // Creating Utility Matrix. It is of form number of users X number of movies
       /* 
        Matrix UtilityMatrix=new Matrix(usersSize,moviesSize);
        List<Tuple2<Integer, List<Double>>> utils=utility.collect();
    	for(int h=0;h<utils.size();h++)
    	{
    		List<Double> lis=utils.get(h)._2;
    		for (int g=0;g<lis.size();g++)
    		{
    			UtilityMatrix.set(h, g, lis.get(g));
    		}
    	}
    	double[][] arr=UtilityMatrix.getArray();
    	for(int f=0;f<usersSize;f++)
    	{
    		System.out.println();
    		for(int s=0;s<moviesSize;s++)
    		{
    			System.out.print(arr[f][s]);
    			System.out.print('\t');
    		}
    	}
    	*/
				...
				while(t.hasNext())
				{
					...
					for(int i=0;i<ratinglist.size();i++)
					{
						...
						if((ratinglist.get(i)==0))
						{
							...
							//List<List<Double>> movierating=new ArrayList<List<Double>>();
							/*
							...
							Collections.sort(movierating, new Comparator<List<Double>>() {
